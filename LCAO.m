...
%% GRÀFICS
figure('Name', 'Mètode LCAO - Energia', 'NumberTitle', 'off');
hold on;

plot(dosRa0, E_LCAO, 'b-', 'LineWidth', 1.5, 'DisplayName', 'Enllaçant (a=1)');
plot(dosRa0, E_anti, 'r-', 'LineWidth', 1.5, 'DisplayName', 'Antienllaçant (a=-1)');
plot([0 10], [E1s E1s], 'k--', 'DisplayName', 'E_{1s}');
...
figure('Position', [100, 100, 1200, 500], ...
       'Name', 'Funcions d''ona LCAO 2D', 'NumberTitle', 'off');
...
title(sprintf('Funció d''ona enllaçant (2R = %.4f a_0)', dosRa0_min), 'FontSize', 14);
axis equal;
colormap(jet);
...
xlabel('x (a_0)', 'FontSize', 12);
ylabel('z (a_0)', 'FontSize', 12);
title(sprintf('Funció d''ona antienllaçant (2R = %.4f a_0)', dosRa0_min), 'FontSize', 14);
axis equal;
colormap(jet);
